...
// replace id of user
const USERID = 'FILL-THIS'

const exportPostsRoute = '/root/cohost/published'
const asksRoute = '/root/cohost/asks'
...
for await (const postToImportFolder of postsToImportFolder) {
  ...
  for await (const block of blocks) {
    switch (block.type) {
      ...
      case 'attachment':
      case 'attachment-row': {
        const attachments = block.attachment
          ? [block.attachment]
          : (block.attachments?.map((elem) => elem.attachment) as Attachment[])
        for await (const attachment of attachments) {
          try {
            ...
            const newLocation =
              'uploads/' + generateRandomString() + '.' + fileName.split('.')[fileName.split('.').length - 1]
