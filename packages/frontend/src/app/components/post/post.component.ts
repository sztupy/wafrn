...
export class PostComponent implements OnInit, OnDestroy, OnChanges {
  ...
  async handlePostActions(action: HotkeyAction) {
    ...
    switch (action) {
      case 'likePost':
        await this.bottomReplyBar()?.toggleLike(this.finalPost)
        break
      case 'rewootPost':
        await this.bottomReplyBar()?.toggleReblog(this.finalPost)
        break
      case 'replyPost':
        await this.bottomReplyBar()?.replyPost(this.finalPost)
        break
      case 'quotePost':
